...
func checkListPointers[E any](t *testing.T, l *List[E], es []*Element[E]) {
	...
	if len(es) == 0 {
		if l.root.next != nil && root != l.root.next ||
			l.root.prev != nil && root != l.root.prev {
	...
	var prev, Prev *Element[E] = root, nil
	var next, Next *Element[E]
	for i, e := range es {
		if p := e.prev; prev != p {
...
func checkList[E comparable](t *testing.T, l *List[E], es []E) {
	if !checkListLen(t, l, len(es)) {
		return
	}
